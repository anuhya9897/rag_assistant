...
def main():
    ...
    if not os.path.isdir(KB_DIR):
        raise RuntimeError(
            f"KB_DIR does not exist: {KB_DIR}. Aborting to avoid DB changes."
        )
    ...
    if len(documents) == 0:
        raise RuntimeError(
            "No documents loaded from KB_DIR. Aborting to avoid deleting DB contents. "
            "Check KB_DIR path and INCLUDE_EXTS."
        )
